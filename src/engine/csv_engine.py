import src.engine.datastorage.metadata
...
class csv_engine:
    def __init__(self, animal: str, framerate: int, csv_path: str,
        ...
        if self.resample_fps is not None:
            ...
            if verbose:
                print(f"csv_engine: Resampling video to {self.resample_fps}fps, frames resample by {frame_resample_by}th index")
            framerate = self.resample_fps

        self.meta = engine.datastorage.metadata.metadata(self, animal, csv_path, framerate, frame_resample_by,
                                                         bodyparts, coordinate_system, use_likelihood, verbose,
                                                         start_index, end_index)
        self.generator = engine.generator.generator(self, verbose)
